...
export default function Home() {
  ...
  // Real usage this hour, read straight from the server's response headers,
  // not guessed client side. Starts visible at 0/4.
  const [usage, setUsage] = useState<{ count: number; max: number }>({ count: 0, max: 4 });
  ...
  const readUsageFromHeaders = (res: Response) => {
    const count = res.headers.get("X-RateLimit-Count");
    const max = res.headers.get("X-RateLimit-Max");
    if (count !== null && max !== null) {
      setUsage({ count: Number(count), max: Number(max) });
    }
  };
        ...
        <main className="relative flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-10">
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
            ...
            <div className="lg:col-span-3 flex flex-col gap-4">
              ...
              {usage && (
                <div className="flex items-center justify-center gap-1.5 text-[11px] text-gray-400 font-medium" style={{ fontFamily: "var(--font-mono)" }}>
                  <span
                    className={`w-1.5 h-1.5 rounded-full ${usage.count >= usage.max ? "bg-red-400" : "bg-[#3db534]"}`}
                  />
                  Sessions this hour: {Math.min(usage.count, usage.max)}/{usage.max}
                </div>
              )}
            </div>
          </div>
